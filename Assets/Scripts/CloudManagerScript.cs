...
public class CloudManagerScript : MonoBehaviour {
    ...
    void Update() {
        if (GameObject.FindGameObjectsWithTag("Cloud").Length < cloudAmt) {
            GameObject newCloud = Instantiate(clouds[Random.Range(0, clouds.Length)], new Vector3(GameObject.Find("Camera").transform.position.x + Random.Range(-maxX, maxX), Random.Range(minAltitude, maxAltitude), 0f), Quaternion.identity);
