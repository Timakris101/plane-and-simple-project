...
public class GForcesScript : NetworkBehaviour {
    ...
    void Update() {
        ...
        if (overGPlaneToDeath() && !destroyed) {
            ...
            if (GameObject.Find("NetworkManager") != null && IsClient) {
                GameObject.Find("NetworkManager").GetComponent<MultiplayerCreateAndDestroy>().destroy(effect, 10f);
            } else {
                Destroy(effect, 10f);
            }
    ...
    public bool ableToRollover() {
        if (GetComponent<PlaneController>() == null) return false;
        ...
        if (GetComponent<PlaneController>().altitudeFromTerrain() < 20f && 
